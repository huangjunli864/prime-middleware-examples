...
package example.ehealth.services.assistance.hsc.resources;
...
import org.prime.extensions.goprime.*;
...
public class HealthServiceCenterResource extends PrimeResource{
...
//	}
	...
	public JaxbRepresentation<ServiceInfoResponse> toXml(){
		...
		try{
			AssemblyUtilityMonitor ri = base.getAssemblyManager().getAssemblyBinding(this.getCURI(), new AURI("http://www.erc-smscom.org/ontologies/2013/4/eHealth#FirstAid"));
	...
	public Representation toHTML(){
    	...
    	String rep = "<html> \n"
				   ...
				   + "<title>eHealth - Health Care Assistance</title></head>\n"
	...
	@Post
	public JaxbRepresentation<ServiceInfoResponse> call(Representation entity) throws IOException {
		...
		if (what.equals("techical"))
			try{
				AssemblyUtilityMonitor ri = ((GoPrimeApplication) getBase()).getAssemblyManager().getAssemblyBinding(this.getCURI(), new AURI("http://www.erc-smscom.org/ontologies/2013/4/eHealth#TechnicalService"));
				log.debug("Type: " + ri.getType() +"-> Instance: " + ri.getInstance());
