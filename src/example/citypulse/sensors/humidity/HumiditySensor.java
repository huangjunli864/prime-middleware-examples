...
package example.citypulse.sensors.humidity;
...
import example.citypulse.sensors.humidity.resources.HumiditySensorResource;
import example.citypulse.sensors.utility.GPS;
import example.citypulse.sensors.utility.Humidity;


public class HumiditySensor extends PrimeApplication{
    ...
    public HumiditySensor(String id, int httpPort) throws Exception{
		super(id, httpPort);
		
		gps = new GPS();
		h = new Humidity();
		
    	myinfo = new SensorInfo(gps.getGeoInfo(),  String.valueOf(h.getValue()), "humidity");
...
//    }
    ...
    public void loadResource(Class<? extends PrimeResource> resource){
    	try {
			...
			tempAURI = d.getAURI();
			myinfo.setName(tempCURI.toString());
	...
	public void checkSensorValue(long sleep){
		while(true){
			try {
				if (this.myinfo.getValue().compareTo(String.valueOf(h.getValue())) != 0 ){
					notify(tempCURI, tempAURI, myinfo);
				    this.myinfo.setValue(String.valueOf(h.getValue()));
				}
				Thread.sleep(sleep);
	...
	public static void main(String[] args) throws Exception {
    	...
    	HumiditySensor sensor = new HumiditySensor(name, httpPort);
        ...
        sensor.loadResource(HumiditySensorResource.class);
